 ...
 * ADMIN OPTIONS
 */
function AtozSitesAdminMenu() {
    add_options_page('S3 Media Storage', 'S3 Media Storage', 'manage_options', 'AtozSitesAdminMenu', 'AtozSitesAdminContent');
}

add_action('admin_menu', 'AtozSitesAdminMenu');

function AtozSitesAdminContent() {
    if (isset($_POST['submit'])) {
        ...
        if (!empty($errors)) {
            ...
            ?>
            <div class="error"><p><strong><?php _e($msg, 'AtozSites' ); ?></strong></p></div>
            <?php
        } else {
            // No errors!
            $settings = array(
            ...
            $ret = update_option('AtozSites_settings', $settings);

            ?>
            <div class="updated"><p><strong><?php _e('Settings Saved!', 'AtozSites' ); ?></strong></p></div>
            <?php
        }
    }
    ...
    if (isset($_POST['move_files']) || isset($_POST['copy_files'])) {
        ...
        if (isset($_POST['selected']) && is_array($_POST['selected'])) {
            ...
            foreach ($_POST['selected'] as $id) {
                $ret = AtozSites::updateAttachmentMetadata(array('AtozSites_move' => $move), $id);
                if ($ret) {
                    $success_count++;
                } else {
                    $error_count++;
            ...
            ?>
            <div class="updated"><p><strong><?php _e(number_format($success_count) . ' File(s) '.$label.'!', 'AtozSites' ); ?></strong></p></div>
            <?php
            if ($error_count > 0) {
                ?>
                <div class="error"><p><strong><?php _e(number_format($error_count) . ' File(s) Could Not Be '.$label.'!', 'AtozSites' ); ?></strong></p></div>
                <?php
            }
        }
    }
    ...
    $settings = json_decode(get_option('AtozSites_settings'), true);
...
<div class="wrap">
	...
	<div id="poststuff">
		...
		<h3><?php _e('Settings'); ?></h3>
            <div class="inside">
            <form id="AtozSites-config" method="post" action="" enctype="multipart/form-data">

                <table class="form-table">
                    <tbody>
                        <tr>
                            <th><label for="key"><?php _e("S3 Bucket Name:", 'AtozSites' ); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th><label for="key"><?php _e("S3 Bucket Path:", 'AtozSites' ); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th><label for="key"><?php _e("Delete Local Files:", 'AtozSites' ); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th><label for="key"><?php _e("Delete from S3:", 'AtozSites' ); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th><label for="key"><?php _e("Expires:", 'AtozSites' ); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th><label for="key"><?php _e("CloudFront Domain Name:", 'AtozSites' ); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th><label for="key"><?php _e("Protocol:", 'AtozSites' ); ?></label></th>
                            <td>
    ...
    <div id="poststuff">
		...
		<h3><?php _e('Library Files'); ?></h3>
            <div class="inside">
            <form id="AtozSites-transfer" method="post" action="" enctype="multipart/form-data">
                ...
                global $wpdb;
                $page = isset($_GET['AtozSites_page']) ? (int) $_GET['AtozSites_page'] : 1;
                $limit = 100;
                $offset = ($limit * $page) - $limit;

                $sql = "SELECT COUNT(1) as count
                        FROM {$wpdb->posts}
                        LEFT JOIN {$wpdb->postmeta} ON {$wpdb->postmeta}.post_id = {$wpdb->posts}.ID AND {$wpdb->postmeta}.meta_key = 'AtozSites_file'
                        WHERE 1=1
                ...
                $page_action = AtozSites::paginationUrl();
                ...
                <table class="wp-list-table widefat">
                    ...
                    <tfoot>
                        <tr>
                            <th colspan="5" scope="row"><p style="float:left"><?php echo AtozSites::pagination($total, $limit, $page, $page_action);?></p>&nbsp;<p style="float:right">Page <?php echo number_format($page);?> of <?php echo number_format($pages);?> Total: <?php echo number_format($total);?> Files</p></th>
                        </tr>
                    </tfoot>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?php $s3_url = AtozSites::attachmentUrl($file->guid, $file->ID); echo $s3_url == $file->guid ? '' : '<a href="'.$s3_url.'" target="_blank">'.$s3_url.'</a>';?></td>
